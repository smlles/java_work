...
public class Ex03_string {
	public static void main(String[] args) {
		...
		//indexOf(char ch)
		//인자로 전달된 글자의 index값을 반환한다.
		//왼쪽부터 오른쪽으로 탐색
		//같은 글자가 여러개라면, 가장 처음 만나는 글자의 index 반환
		System.out.println("글자 M의 위치 : "+str.indexOf('M'));
		
		//charAt(int index)
		//인자로 전달된 인덱스에 해당하는 글자 반환
		System.out.println("4번 index의 글자 : " +str.charAt(4));
		
		//substring(int offet, int end)
		//일정 인덱스만큼 잘라서 반환 offet이상  end미만
		System.out.println(str.substring(0,5));
		
		//split()
		//인자로 전달되는 값을 구분자로 문자열을 분할하여 배열 형태로 반환
		...
		//startWith()
		//인자로 전달된 문자열로 시작하는지 판별
		System.out.println(str.startsWith("Kim"));
		
		//endWith()
		//읹자로 전달된 문자열로 끝나는지 판별
