...
public class Ex02_arrays {
	...
	public static void main(String[] args) {
		...
		//arrays클래스를 이용하면, 배열의 기능을 더 쉽게 사용 할 수 있다.
		
		//Scanner, Random은 객체 생성 후 메서드 사용
		//Arrays.함수명()으로 기능을 호출함
		
		//1.배열의 출력 
		//toString()
		//반복문 없이 배열을 출력 할 수 있도록 도와준다.
		//배열에 정의된 값을 문자열 형태로 변환하여 출력한다.
		...
		System.out.println(Arrays.toString(arr));
		Arrays.sort(arr);//반환 값 없음
		System.out.println("정렬 후");
		System.out.println(Arrays.toString(arr));
		
		//comparator.reverseOrder()로 내림차순 정렬이 되나
		//기본 자료형 배열론 불가능
		//기본 자료형의 클래스 타입인 Wrapper클래스 이용
		...
		//배열 복사
		//자바에서 배열을 생성하면 길이를 변경 할 수 없으므로
		//더 많은 데이터를 저장하거나, 기존 배열과 같은 배열을 새로 만드려면
		//배열을 복사해야한다.
		//배열을 복사하는 방법에는 얕은 복사, 깊은 복사가 있다.
		
		//얕은복사(Shallow Copy) : 복사된 배열이나, 원본 배열이 변경될 때
		//서로 간의 값이 함께 변경된다.
		
		//깊은복사(Deep Copy) : 복사된 배열이나 원본 배열이 변경되어도
		//서로간의 값은 바뀌지 않는다.
		
		int[] arr01 = {1,2,3};
		//arr01은 배열 {1,2,3} 의 주소값을 가짐
		...
		//깊은 복사
		//배열의 깊은 복사는, 반복문을 통해 직접 값을 넣거나
		//Array클래스 또는 System 클래스가 가진 기능을 이용
		...
		//다차원 배열
//		/2차원 이상의 배열을 의미하며, 배열의 요소로 또 다른 배열을 가진다
		
		// 2차원 배열은 요소로 1차원 배열을 가진다.
		//int [] [] arr = new int [2차원 배열의 크기] [1차원 배열의 크기] 
		int[][] arr2 = { {1,2,3} , {4,5,6} , {7,8,9} } ;
		System.out.println( arr2[0][0]);
		
		//요소로 들어가는 1차원 배열의 크기를 지정하지 않고 생성 할 수 있다.
		//각 배열의 크기를 각각 선언하여 사용 할 수 있다.
		int[][] arr3 = new int[3][];
		arr3[0] =new int[2];
		arr3[1] =new int[3];
		arr3[2] =new int[1];
